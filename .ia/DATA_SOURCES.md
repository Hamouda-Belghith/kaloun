...
- **Pagination réelle de cette édition** (numérotation interne = position dans le fichier, utilisée comme numéro de page dans l'app) :
  ...
  - Pages 606–627 : pages de fin. Page 606 = colophon "التعريف بهذا المصحف الشريف" confirmant que cette édition suit **la lecture de Qaloun via Nassif, transmise par Nafi' al-Madani** ("رواية قالون من طريق أبي نشيط عن نافع المدني"), rasm et arrêts conformes aux mosahef imprimés de référence, décompte selon l'école coufie (6236 versets). Pages 607–614 : présentation de l'édition (رسم/ضبط). Pages 615–626 : règles de tajwid (waqf, noun/mim sakina, idgham, qalb, ikhfa, makharij). Page 627 : 4e de couverture. Inspecté visuellement le 2026-09-24 (voir session 9 de [PROGRESS.md](PROGRESS.md)) suite à une page trouvée à l'envers — pas d'autre anomalie sur cette plage.
  - ⚠️ **Page 620 corrigée le 2026-09-24** : scannée à l'envers dans le PDF source (rotation 180°, signalé par l'utilisateur). Corrigée en local par simple rotation de l'image extraite (`app/assets/pages/page_0620.webp`), sans autre retouche. Une vérification automatique de rotation sur l'ensemble des 627 pages n'a pas été faite (pas d'outil de détection en place) — les pages 3–605 ont été vues en détail lors de la vérification des sourates (session 6) sans qu'aucune autre anomalie de ce type saute aux yeux, mais ce n'est pas une garantie formelle. À surveiller pendant la relecture humaine ([MANUEL.md](../MANUEL.md) étape 5).
